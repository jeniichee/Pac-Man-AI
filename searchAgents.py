...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                next_corner = list(visited_corner)
                if next_node in self.corners and next_node not in next_corner: 
                        next_corner.append(next_node)
                        
                #update the state with the new remaining corners
                #action in the for loop and cost of 1
                successors.append(((next_node, next_corner), action, 1))
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    while len(corners_unvisited) > 0: 
        heuristic_cost, corner = \
            min([(manhattan(currPos, corner), corner) for corner in corners_unvisited])
        currPos = corner
        total += heuristic_cost
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    manhatten = util.manhattanDistance
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state: Tuple[int, int]):
        ...
        "*** YOUR CODE HERE ***"
        distance, goal = min([(manhattan(state, goal), goal) for goal in food])
        return state == goal
